...
const SellerAuthState = (props) => {
  ...
  const getSellerProducts = async () => {
    ...
    try {
      ...
      dispatch({ type: ADD_IMAGE_FAIL, payload: "Failed to fetch seller products" });
    }
  };
  ...
  const updateImage = async (formData) => {
    ...
    try {
      await axios.put(`/api/seller/profile/image`, formData, config);
      dispatch({ type: UPDATE_IMAGE });
    } catch {
      dispatch({ type: ADD_IMAGE_FAIL, payload: "Failed to update image" });
    }
  };
  ...
  const updateProfile = async (user) => {
    ...
    try {
      ...
      dispatch({ type: ADD_IMAGE_FAIL, payload: "Failed to update profile" });
    }
  };
